...
export const createBkashPayment = async (amount: number, smsQty: number) => {
  try {
    const res = await fetch(`${getEndpoint('bkash_relay.php')}?action=create`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const getSMSBalance = async (): Promise<number> => {
  try {
    const res = await fetch(getEndpoint('manage_sms_balance.php'));
    if (!res.ok) return 0;
...
export const saveSMSBalance = async (balance: number) => {
  try {
    await fetch(getEndpoint('manage_sms_balance.php'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export const sendActualSMS = async (config: SMSConfig, phone: string, message: string, cost: number = 1): Promise<{success: boolean, message: string}> => {
  try {
    ...
    const response = await fetch(getEndpoint('send_sms.php'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({

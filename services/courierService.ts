...
const fetchSetting = async (key: string) => {
  try {
    const res = await fetch(`${getEndpoint('settings.php')}?key=${key}`);
...
export const fetchAllLocalTracking = async (): Promise<any[]> => {
  try {
    const res = await fetch(getEndpoint('local_tracking.php'));
    if (!res.ok) return [];
    const text = await res.text();
    try {
      const data = JSON.parse(text);
...
export const saveTrackingLocally = async (orderId: string, trackingCode: string, status: string, courier?: 'Steadfast' | 'Pathao') => {
  ...
  try {
    const response = await fetch(getEndpoint('local_tracking.php'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
export const getCourierBalance = async () => {
  ...
  try {
    const response = await fetch(`${getEndpoint('courier.php')}?action=balance`, {
      method: 'GET',
      headers: {
...
export const createSteadfastOrder = async (order: Order) => {
  ...
  try {
    const response = await fetch(`${getEndpoint('courier.php')}?action=create`, {
      method: 'POST',
      headers: {
        'Api-Key': config.apiKey,
        'Secret-Key': config.secretKey,
...
export const getDeliveryStatus = async (trackingCode: string) => {
  ...
  try {
    const response = await fetch(`${getEndpoint('courier.php')}?action=status&tracking_code=${trackingCode}`, {
      method: 'GET',
      headers: {
        'Api-Key': config.apiKey,
        'Secret-Key': config.secretKey,

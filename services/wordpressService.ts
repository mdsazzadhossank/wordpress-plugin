...
export const uploadImageToWP = async (file: File): Promise<{ success: boolean; url?: string; error?: string }> => {
  ...
  // Use proxy to avoid CORS/Auth issues when uploading to external WP
  // Or if on same domain, we can use the plugin endpoint directly if implemented
  // Assuming external:
  ...
  try {
    ...
    if (response.status === 404) {
        throw new Error("Plugin not installed. Please install 'BdCommerce Connect' plugin on the target WordPress site.");
    }
